...
import { wallpaperImageFor } from '../../data/wallpapers';
import { useDynamicLook } from '../Desktop/DynamicWallpaper';
...
interface Props {
  /** Called the moment the visitor unlocks, so the desktop can mount under the fading screen. */
  onUnlock?: () => void;
  /** Called once the screen has faded out and can be removed. */
  onLogin: () => void;
}

/** How long the lock screen takes to fade out over the desktop. */
export const LOGIN_LEAVE_MS = 500;

/** Shared unlock behaviour: any key, a click on the prompt, or a swipe up on phones. */
function useUnlock(onLogin: () => void, onUnlock?: () => void, listenForKeys = true) {
  ...
  const enter = useCallback(() => {
    ...
    setLeaving(true);
    onUnlock?.();
    setTimeout(onLogin, LOGIN_LEAVE_MS);
  }, [onLogin, onUnlock]);
...
/** The wallpaper image behind the lock screen: the current frame of a dynamic wallpaper. */
function useLockWallpaper() {
  const { wallpaper } = useSettings();
  const look = useDynamicLook(wallpaper);
  return { wallpaper, image: wallpaperImageFor(wallpaper, look) };
}

/* ── macOS ─────────────────────────────────────────────────────────────── */
function MacLogin({ onLogin, onUnlock }: Props) {
  const { settings } = useSettings();
  const { image } = useLockWallpaper();
  const [visible, setVisible] = useState(false);
  const { leaving, enter } = useUnlock(onLogin, onUnlock);
...
/* ── Windows 11 ────────────────────────────────────────────────────────── */
function WindowsLogin({ onLogin, onUnlock }: Props) {
  const { settings } = useSettings();
  const { image } = useLockWallpaper();
  ...
  const { leaving, enter } = useUnlock(onLogin, onUnlock, stage === 'signin');
  const { now, time } = useLockClock();
...
function IphoneLock({ onLogin, onUnlock }: Props) {
  const { image } = useLockWallpaper();
  const [visible, setVisible] = useState(false);
  const { leaving, enter, swipe } = useUnlock(onLogin, onUnlock);
  const { now, time } = useLockClock();
...
function AndroidLock({ onLogin, onUnlock }: Props) {
  const { settings } = useSettings();
  const { wallpaper, image } = useLockWallpaper();
  const [visible, setVisible] = useState(false);
  const { leaving, enter, swipe } = useUnlock(onLogin, onUnlock);
...
export default function Login(props: Props) {
  ...
  // The screen fades in from, and (when locking) out to, black rather than the page colour.
  usePageBackground('#000');
  if (os === 'windows') return <WindowsLogin {...props} />;
  if (os === 'ios') return <IphoneLock {...props} />;
  if (os === 'android') return <AndroidLock {...props} />;
  return <MacLogin {...props} />;
}
